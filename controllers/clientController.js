 const redis = require('redis');
  ...
  try {
    ...
    const clients = clientIds.map(clientId => {
      ...
      const namePart = parts[2]?.split("'")[1]; // Extrae el nombre
      return {
   ...
   exports.addNewClient = async (req, res) => {
    ...
    // Función para obtener clientes de una sucursal
    exports.getClientesSucursal = async (req, res) => {
      ...
      try {
          ...
          const sucursalDetails = await redisClient.sMembers(redisKey);
  
          if (!sucursalDetails || Object.keys(sucursalDetails).length === 0) {
              return res.status(404).json({ message: 'sucursal no encontrada' });
          }
  
          // Retornar los detalles del producto si se encuentra
          return res.status(200).json(sucursalDetails);
      } catch (error) {
